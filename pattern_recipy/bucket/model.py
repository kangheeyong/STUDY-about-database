...
class BucketPattern(Document):
    ...
    def make_dataset(cls):
        ...
        for i in range(100, 0, -1):
            ...
            cls.objects.create(date_time=days, measurements=embd, count=len(embd))

    @classmethod
    def get_obj(
        cls, date_time: datetime = NOW - timedelta(days=1)
    ) -> Optional[BucketPattern]:
        try:
    ...
    def update_using_pymongo(cls, temperature: int, date_time: datetime = NOW):
        ...
        cls._get_collection().update_one(_filter, _update, upsert=True)
    ...
    def delete_using_mongoengine_v1(cls, date_time: datetime = NOW - timedelta(days=1)):
        ...
        for idx in range(len(obj.measurements) - 1, -1, -1):
            if obj.measurements[idx].time_stamp < NOW - timedelta(days=1) + timedelta(
                seconds=DELETE_COUNT
            ):
    ...
    def delete_using_pymongo(cls, date_time: datetime = NOW - timedelta(days=1)):
        ...
        _update = [
                        ...
                        "$filter": {
                            ...
                            "cond": {
                                "$gte": [
                                    ...
                                    NOW
                                    - timedelta(days=1)
                                    + timedelta(seconds=DELETE_COUNT),
                                ]
                            },
                        }
                    }
            ...
            {"$set": {"count": {"$size": "$measurements"}}},
        ]
        cls._get_collection().update_one(_filter, _update)
